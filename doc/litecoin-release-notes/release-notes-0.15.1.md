Tropiky Core version *0.15.1* is now available from:

  <https://download.tropiky.org/tropiky-0.15.1/>
...
A number of changes to the way Tropiky Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 trop per kB (0.00001 TRK). This is to prepare for dropping the minimum transaction fee to 0.1 trop per kB (0.0001 TRK) in the next release.
