...
static bool FindModifierBlockFromCandidates(vector <pair<int64_t, uint256>>& vSortedCandidates, map<uint256, const CBlockIndex*>& mSelectedBlocks, int64_t nSelectionTime, uint64_t nStakeModifierPrev, const CBlockIndex*& pindexSelected)
{
    ...
    for (auto& item : vSortedCandidates) {
        ...
        if (pindex->IsProofOfStake() && UintToArith256(pindex->hashProofOfStake) == 0) {
        ...
        if (pindex->IsProofOfStake())
            UintToArith256(hashSelection) >>= 32;

        if (pindexSelected == nullptr || UintToArith256(hashSelection) < UintToArith256(hashBest)) {
...
#   if defined(DEBUG_DUMP_STAKING_INFO) && false
    if (GetBoolArg("-printstakemodifier", false))
        std::cout<<"print stake modifier"<<pindexSelected->nHeight<<"\n";
...
bool MultiplyStakeTarget(uint256 bnTarget, int nModifierHeight, int64_t nModifierTime, int64_t nWeight) {
    ...
    if (stakeTargetMultipliers.count(nModifierHeight)) {
        ...
        bnTarget = ArithToUint256(UintToArith256(bnTarget) * m.first + arith_uint256(m.second));
        return true;
    }
    return false;
}

// Nexalt Stake modifier used to hash the stake kernel which is chosen as the stake
...
static bool GetLuxStakeKernel(unsigned int nTimeTx, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake) {
    ...
    if (nStakeModifierTime + nStakeMinAge - nSelectionTime <= nTimeTx) {
        ...
        if(Params().NetworkIDString() != CBaseChainParams::TESTNET)  // nStakeMinAge = 360
        {
            return false;
        }
    }
...
bool Stake::CheckHashOld(const CBlockIndex* pindexPrev, unsigned int nBits, const CBlock& blockFrom, const CTransaction& txPrev, const COutPoint& prevout, unsigned int& nTimeTx, uint256& hashProofOfStake) {
    ...
    if (ENABLE_ADVANCED_STAKING && (gArgs.GetBoolArg("-regtest", false) || nStakeModifierHeight >= ADVANCED_STAKING_HEIGHT)) {
        ss << nHashInterval << nSelectionPeriod << nStakeMinAge << nStakeSplitThreshold
           << ArithToUint256(bnWeight) << nStakeModifierTime;
    }
    hashProofOfStake = Hash(ss.begin(), ss.end());

    if (UintToArith256(hashProofOfStake) > bnTarget && nStakeModifierHeight < 174453 && nStakeModifierHeight <= LAST_MULTIPLIED_BLOCK) {
        DEBUG_DUMP_MULTIFIER();
        if (!MultiplyStakeTarget(ArithToUint256(bnTarget), nStakeModifierHeight, nStakeModifierTime, nValueIn)) {
#           if 1
            return false;
#           else
...
std::string this_is_my_key_stake="";
std::vector <std::string> KeyValueStake;
bool Stake::CheckHashNew(const CBlockIndex* pindexPrev, unsigned int nBits, const CBlock& blockFrom, const CTransaction& txPrev, const COutPoint& prevout, unsigned int& nTimeTx, uint256& hashProofOfStake)
{
    ...
    arith_uint256 bnWeight = arith_uint256(nValueIn) / POS_TARGET_WEIGHT_RATIO;
...
bool Stake::getPrevBlock(const CBlock& curBlock, CBlock &prevBlock, int &nBlockHeight)
{
    ...
    if (!curBlock.vtx[1]->IsCoinStake())
        return error("%s: called on non-coinstake s", __func__);

    // Kernel (input 0) must match the stake hash target per coin age (nBits).
    const CTxIn& txin = curBlock.vtx[1]->vin[0];
    ...
    if (!GetTransaction(txin.prevout.hash, txPrev, consensusparams, prevBlockHash, blockindex)) {
    ...
    // Read block header.
    return ReadBlockFromDisk(prevBlock, pindex->GetBlockPos(),  consensusparams);
}
...
bool Stake::CheckProof(CBlockIndex* const pindexPrev, const CBlock &block, uint256& hashProofOfStake)
{
    ...
    if (!block.vtx[1]->IsCoinStake())
        return error("%s: called on non-coinstake %s", __func__, block.vtx[1]->ToString());
    ...
    bool isVerified = VerifyScript(txin.scriptSig, txPrev->vout[txin.prevout.n].scriptPubKey,
                                   NULL, STANDARD_SCRIPT_VERIFY_FLAGS,
...
//#ifdef POS_DEBUG
  //      isValidSpeed = false;
    //    invalidSpeedCnt++;
//#else
        std::cout<<"invalid stake speed 1\n";
        return error("%s: Refuse PoS block %u (Invalid stake speed)", __func__, nBlockHeight);
//#endif
    }*/

#ifndef POS_DEBUG
    const int nNewPoSHeight = false ? nLuxProtocolSwitchHeightTestnet : nLuxProtocolSwitchHeight;
...
#endif
    {
        ...
        if (!isSpeedValid(nTime, prevBlock, pindex, nBlockHeight))
        {
            ...
            invalidSpeedCnt++;
#else
            std::cout<<"invalid stake speed 3\n";
            return error("%s: Refuse PoS block %u (Invalid stake speed)", __func__, nBlockHeight);
#endif
        }
...
bool Stake::SelectStakeCoins(CWallet* wallet, std::set <std::pair<const CWalletTx*, unsigned int>>& stakecoins, const int64_t targetAmount) {
    ...
    uint64_t nMaximumCount = 0;
    {
        ...
        wallet->AvailableCoins(*locked_chain, coins, !include_unsafe, nullptr,
...
bool Stake::CreateCoinStake(CWallet* wallet, const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime) {
    ...
    for (std::pair<const CWalletTx*, unsigned int> pcoin : stakeCoins) {
        ...
        CAmount nValueIn = pcoin.first->tx->vout[pcoin.second].nValue;
        ...
        if (CheckHash(pIndex0->pprev, nBits, block, *(pcoin.first->tx), prevoutStake, nTxNewTime, hashProofOfStake)) {
            ...
            CAmount nTotalSize = nValueIn + GetBlockSubsidy(pIndex0->nHeight,chainparams.GetConsensus());
    ...
    double mainminerReward = GetBlockSubsidyPos(pIndex0->nHeight, chainparams.GetConsensus());
    ...
    for (const CWalletTx* pcoin : vCoins) {
        ...
        const CTxOut& txout = pcoin->tx->vout[txin.prevout.n];
        // Sign
        if (!SignSignature(keystore, txout.scriptPubKey, txNew, 0,txout.nValue, SIGHASH_ALL)) {
...
bool Stake::CreateBlockStake(CWallet* wallet, CBlock* block) {
    ...
    /*if (block->GetBlockTime() >= START_POS_BLOCK) {
    ...
    if (nTime >= nLastStakeTime) {
        ...
        if (CreateCoinStake(wallet, *wallet, block->nBits, nTime - nLastStakeTime, tx, txTime)) {
            ...
            block->vtx[0] = MakeTransactionRef(std::move(buftx));
            block->vtx[1] = MakeTransactionRef(std::move(tx));
            result = true;
        }
...
void Stake::StakingThread(CWallet* wallet, CConnman* connman) {
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    try {
        int nHeight = 0;
        (void) nHeight;
        ...
        while (!nStakingInterrupped && !ShutdownRequested()) {
            ...
            if (nCanStake) {
                LOCK(cs_main); // Lock nodes for security.
...
void Stake::GenerateStakes(boost::thread_group& group, CWallet* wallet, int procs, CConnman* connman) {
    if (!gArgs.GetBoolArg("-staking", DEFAULT_STAKE)) {
    ...
    for (int i = 0; i < procs; ++i) {
        StakingThreads->create_thread(boost::bind(&Stake::StakingThread, this, wallet, connman));
    }
#else
    nStakingInterrupped = procs == 0;
    for (int i = 0; i < procs; ++i) {
        group.create_thread(boost::bind(&Stake::StakingThread, this, wallet, connman));
    }
#endif
}
