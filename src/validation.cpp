...
#if defined(NDEBUG)
# error "Tropiky cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Tropiky Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s TRK additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidyPos(int nHeight, const Consensus::Params& consensusParams)
{
    int halvings = nHeight / consensusParams.nSubsidyHalvingInterval;
    // Force block reward to zero when right shift is undefined.
    if (halvings >= 64)
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        for (size_t o = 0; o < tx.vout.size(); o++) {
            if (!tx.vout[o].scriptPubKey.IsUnspendable()) {
                ...
                if (!is_spent || tx.vout[o].ToString() != coin.out.ToString() || pindex->nHeight != coin.nHeight || is_coinbase != coin.fCoinBase ) {
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (block.IsProofOfStake()) {
        CAmount blockReward = GetBlockSubsidyPos(pindex->nHeight, chainparams.GetConsensus());
        uint64_t nCoinAge;
